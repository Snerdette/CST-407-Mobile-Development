...
fun getTopThree(listOfProgrammers: List<Programmer>): List<Programmer> {
	return listOfProgrammers.sortedWith(compareByDescending { it.level}).take(3)
}
...
fun getRandomProgrammer(listOfProgrammers: List<Programmer>): Programmer {
	return listOfProgrammers.elementAt(listOfProgrammers.size.random())
}
...
fun getRandomProgrammerNull(listOfProgrammers: List<Programmer>): Programmer? {
	//return null
	// Still not sure if this is what I'm supposed to be doing here...
	return listOfProgrammers.getOrElse(listOfProgrammers.size.random(), {null})
}
